...
from flask import redirect, jsonify, url_for, flash, jsonify
...
# works like a dictionary and we can  store values in it for the longevity of a
...
cloudinary.config(cloud_name="aui3", api_key="227915768443584",
...
def showLogin():
    ...
    # a (cross-site request forgery) forger will have to guess this key
    # if we wants to make requests on the user's behalf.
    # check to make sure the user and login_session have the same state value
...
def gconnect():
    ...
    # check if the token that the client sends to the server
    # request.args.get('state') is the same as the token that the server
    # sent the client login_session['state']
    # ensures that the client is making the request and not a malicious script
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    # collect the one time code that the client sends (via the ajax call).
    # this is the code that google sends on authoirzation..
    code = request.data
...
def showCategories():
    ...
    recent_list = session.query(Resource).order_by(
                                Resource.date_time.desc()).limit(5)

    if 'user_id' in login_session:
        credentials = login_session.get('credentials')
        print "access %s", credentials.access_token
        return render_template("bookmark_categories.html",
        ...
        return render_template("bookmark_categories.html",
                               recent_list=recent_list,
                               category_list=category_list, user_id="")
...
def newCategory():
    ...
    if request.method == 'POST':
        newCategory = BookmarkCategory(name=request.form['name'],
...
@app.route('/bookmark_categories/<int:category_id>/edit/',
           methods=['GET', 'POST'])
def editCategory(category_id):
    ...
    editedCategory = session.query(BookmarkCategory).filter_by(
                                   id=category_id).one()
    ...
    if request.method == 'POST':
        if login_session['user_id'] == editedCategory.user_id:
            ...
            flash("Unauthorized to Edit Category!!")
            return redirect(url_for('showCategories'))
    else:
...
@app.route('/bookmark_categories/<int:category_id>/delete/',
           methods=['GET', 'POST'])
def deleteCategory(category_id):
    ...
    toBeDeletedCategory = session.query(BookmarkCategory).filter_by(
                                        id=category_id).one()

    if request.method == 'POST':
        if login_session['user_id'] == toBeDeletedCategory.user_id:
            ...
            flash("Unauthorized to Delete Category!!")
            return redirect(url_for('showCategories'))
    else:
...
def showResources(category_id):
    resources_list = session.query(Resource).filter_by(
        category_id=category_id)
    category_listed = session.query(BookmarkCategory).filter_by(
                                    id=category_id).one()
    category_list = session.query(BookmarkCategory)

    # check if a user is logged and send session information to the
    # template if user present
    if 'user_id' in login_session:
...
@app.route('/bookmark_categories/<int:category_id>/resources/new/',
           methods=['GET', 'POST'])
def newResource(category_id):
    ...
    if request.method == 'POST':
        if request.form["url"]:
            # using the BeautifulSoup module to automatically extract the
            # title of the URl to be added as a bookmark
            soup = BeautifulSoup(urllib2.urlopen(request.form["url"]))
            title = soup.find('title')
            # if the BeautifulSoup module can not extract a title,
            ...
            image = cloudinary.CloudinaryImage(
                    request.form['url'], type="url2png").build_url(
                                    crop="fill",
                                    width=150,
                                    height=200,
                                    gravity="north",
                                    sign_url=True)
            bookmark = Resource(url=request.form["url"],
...
@app.route(
   '/bookmark_categories/<int:category_id>/resources/<int:resource_id>/edit/',
   methods=['GET', 'POST'])
def editResource(category_id, resource_id):
    ...
    toBeEditedResource = session.query(Resource).filter_by(
                                       id=resource_id).one()

    if request.method == 'POST':
        if login_session['user_id'] == toBeEditedResource.user_id:
            if request.form['name']:
            ...
            flash("Unauthorized to edit this bookmark")
            return redirect(url_for('showResources', category_id=category_id))
    else:
        return render_template("edit_resource.html",
                               category_id=category_id,
                               resource_id=resource_id,
                               resource=toBeEditedResource,
                               login_session=login_session)


# Delete a bookmark resource.
@app.route(
 '/bookmark_categories/<int:category_id>/resources/<int:resource_id>/delete/',
 methods=['GET', 'POST'])
def deleteResource(category_id, resource_id):
    ...
    toBeDeletedResource = session.query(Resource).filter_by(
                                        id=resource_id).one()

    if request.method == 'POST':
        if login_session['user_id'] == toBeDeletedResource.user_id:
            session.delete(toBeDeletedResource)
            session.commit()
            flash("Bookmark deleted")
            return redirect(url_for('showResources', category_id=category_id))
        else:
            flash("Unauthorized to delete bookmark")
            return redirect(url_for('showResources', category_id=category_id))

    else:
        return render_template('delete_resource.html',
                               category_id=category_id,
                               resource_id=resource_id,
                               resource=toBeDeletedResource,
                               login_session=login_session)


# Edit  a bookmark note.
@app.route(
    ('/bookmark_categories/<int:category_id>/resources/<int:resource_id>/'
     'edit_notes/'), methods=['GET', 'POST'])
def editNotes(category_id, resource_id):
    ...
    if request.method == 'POST':
        if login_session['user_id'] == resource.user_id:
            resource.notes = request.form['notes']
            session.add(resource)
            session.commit()
            flash("Note Edited")
            return redirect(url_for('showResources',
                                    category_id=category_id))
        else:
            flash("Unauthorized to edit note")
            return redirect(url_for('showResources',
                            category_id=category_id))
    else:
        return render_template("edit_notes.html", category_id=category_id,
...
def delete_bookmarks_in_category(category_id):
    print "here"
    bookmarks_list_del = session.query(Resource).filter_by(
                                       category_id=category_id)
...
def bookmarksInCategory(category_id):
    bookmarks_list = session.query(Resource).filter_by(
                                   category_id=category_id)
...
@app.route(
   '/bookmark_categories/<int:category_id>/resources/<int:resource_id>/JSON')
